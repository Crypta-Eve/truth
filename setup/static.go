...
func PopulateStaticData(c *cli.Context) error {
	...
	// This is commented out so I dont need to redo everything every time....

	client.Store.DeleteStaticData()

	err = populateUniverse(client)
	if err != nil {
		return cli.NewExitError(errors.Wrap(err, "Failed to populate the universe"), 1)
	}

	err = populateTypes(client)
	if err != nil {
		return cli.NewExitError(errors.Wrap(err, "Failed to populate types"), 1)
	}

	err = populateGroups(client)
	if err != nil {
		return cli.NewExitError(errors.Wrap(err, "Failed to populate groups"), 1)
	}

	err = populateCategories(client)
	if err != nil {
		return cli.NewExitError(errors.Wrap(err, "Failed to populate categories"), 1)
	}
...
func populateRegions(client *client.Client) error {
	...
	for _, rr := range regions {
		...
		go func() {
			...
			client.Store.InsertRegion(region)
			waitgroup.Done()
		}()
...
func populateConstellations(client *client.Client) error {
	...
	batchSize := (len(constellations) / numThreads) + 1
	...
	for _, b := range batches {
		...
		go func() {
			for _, r := range batch {
				...
				constellation := store.ESIConstellation{}
				...
				client.Store.InsertConstellation(constellation)
			}
			waitgroup.Done()
		}()
	}
...
func populateSystems(client *client.Client) error {
	...
	batchSize := (len(systems) / numThreads) + 1
...
func populateStars(client *client.Client) error {
	...
	batchSize := (len(starIDs) / numThreads) + 1
...
func populatePlanets(client *client.Client) error {
	...
	batchSize := (len(planetList) / numThreads) + 1
...
func populateMoons(client *client.Client) error {
	...
	batchSize := (len(moonList) / numThreads) + 1
...
func populateAsteroidBelts(client *client.Client) error {
	systemlist, err := client.Store.GetSystems()
	if err != nil {
		return err
	}
	...
	batchSize := (len(beltList) / numThreads) + 1
...
func populateStargates(client *client.Client) error {
	systemlist, err := client.Store.GetSystems()
	if err != nil {
		return err
	}
	...
	batchSize := (len(gateList) / numThreads) + 1
...
func populateStations(client *client.Client) error {
	systemlist, err := client.Store.GetSystems()
	if err != nil {
		return err
	}
	...
	batchSize := (len(stationList) / numThreads) + 1
...
func populateTypes(client *client.Client) error {
	...
	batchSize := (len(types) / (numThreads * 2)) + 1
...
func populateGroups(client *client.Client) error {
	...
	// Because there are so many typeids to fetch, going to double the number of goroutines
	batchSize := (len(groups) / (numThreads * 2)) + 1
...
func populateCategories(client *client.Client) error {
	...
	// Because there are so many typeids to fetch, going to double the number of goroutines
	batchSize := (len(categories) / (numThreads * 2)) + 1
